    ...
    <div>
          ...
          <div style={{ display: 'flex', gap: 4 }}>
            {shifts.map(s => (
              <div key={s.id} style={{
                ...
                {s.id} <span style={{ fontWeight: 400, color: s.color + 'aa' }}>{s.start}-{s.end}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <div style={{ overflowX: 'auto', border: '1px solid var(--border)', borderRadius: 10 }}>
        <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: 700 }}>
          <thead>
            <tr>
              <th style={{ background: 'var(--bg-elevated)', padding: '10px 16px', textAlign: 'left', fontSize: 11, fontWeight: 700, letterSpacing: '0.07em', color: 'var(--text-muted)', borderBottom: '1px solid var(--border)', minWidth: 120 }}>
                員工
              </th>
              {weekDays.map(d => (
                <th key={d.date} style={{
                  ...
                  {d.label}
                  {d.date === today && <div style={{ fontSize: 9, letterSpacing: '0.06em' }}>今天</div>}
                </th>
              ))}
            </tr>
          </thead>
                ...
                {weekDays.map(d => {
                  ...
                  return (
                    <td key={d.date} style={{
                      ...
                      <select
                        value={assigned}
                        onChange={e => setAssignment(emp.id, d.date, e.target.value)}
                        style={{
